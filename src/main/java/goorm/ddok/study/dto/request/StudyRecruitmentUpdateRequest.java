...
@Schema(
        ...
        description = """
            ...
            • mode: online/offline
            • mode=offline → location 필드 필요 (카카오 road_address 매핑)
            • studyType: 한글 라벨(예: "취업/면접")
            • preferredAges: null → 무관(0/0) 저장
            """,
        example = """
        {
          "title": "프리토킹 스터디(수정)",
...
public class StudyRecruitmentUpdateRequest {
    ...
    @Schema(description = "진행 방식 (online / offline). 미전달 시 기존 유지", example = "offline")
    private StudyMode mode;          // online | offline

    @Schema(
            description = "offline일 때만 필요. FRONT에서 카카오 road_address 매핑값 사용",
            example = """
        {
